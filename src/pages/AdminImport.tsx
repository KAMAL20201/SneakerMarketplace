...
function parseCSV(text: string, usdToInr: number): ParsedRow[] {
  ...
  const imageUrlsIdx =
    [indexOf("image_urls"), indexOf("image")].find((i) => i >= 0) ?? -1;
  ...
  const ukInrIndices: { col: string; size_value: string; inrIdx: number }[] =
    isAllSizes
      ? UK_SIZE_COLS.map((s) => ({
          ...s,
          inrIdx: indexOf(`${s.col} INR`),
        })).filter((s) => s.inrIdx >= 0)
      : [];
...
export default function AdminImport() {
  ...
  const toggleAll = (checked: boolean) =>
    setRows((prev) =>
      prev.map((r) => ({
        ...r,
        selected: r.sizeValid && r.price > 0 ? checked : false,
      })),
    );
  ...
  const handleImport = async () => {
    ...
    try {
      for (let i = 0; i < selectedRows.length; i++) {
        ...
        toast.info(
          `Processing ${i + 1}/${selectedRows.length}: ${r.title}${isMulti ? ` (${r.sizes.length} sizes)` : ` (${r.size_value})`}`,
        );
        ...
        try {
          ...
          accumulated.push({
            ...
            size_value: r.size_value || null,
            status: "error",
            reason: rowErr instanceof Error ? rowErr.message : "Network error",
          });
        }
      ...
      const imported = accumulated.filter(
        (r) => r.status === "imported",
      ).length;
        ...
        <Card className="glass-card border-0 rounded-3xl shadow-lg">
          <CardHeader>
            ...
            <p className="text-sm text-gray-600">
              Supports two CSV formats: <strong>single-size</strong> (one row =
              one size) and <strong>all-sizes</strong> (one row = one sneaker
              with UK size columns). For images, run{" "}
              <code className="bg-gray-100 px-1 rounded text-xs">
                node scraper/enrich-images.mjs your.csv
              </code>{" "}
              first to add an{" "}
              <code className="bg-gray-100 px-1 rounded text-xs">
                image_urls
          ...
          <CardContent className="space-y-4">
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-1">
                Single-size columns: Sneaker, Brand, Silhouette, Landed INR,
                Size US, URL
              </p>
              <p className="text-xs text-gray-500">
                All-sizes columns: Sneaker, Brand, Silhouette, Retail $, UK6 $,
                UK6 INR, UK6.5 $, UK6.5 INR … URL
              </p>
              <input
                ref={fileInputRef}
                type="file"
            ...
            <div className="flex flex-wrap items-center gap-3 p-3 bg-amber-50/60 rounded-xl text-sm">
              <span className="text-amber-800 font-semibold whitespace-nowrap">
                USD → INR rate:
              </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-blue-50/60 rounded-xl p-3 text-xs text-blue-700 space-y-1">
                ...
                <ul className="list-disc list-inside space-y-0.5 text-blue-600">
                  <li>
                    <span className="font-mono">Sneaker</span> → title
                  </li>
                  <li>
                    <span className="font-mono">Brand</span> → brand
                  </li>
                  <li>
                    <span className="font-mono">Silhouette</span> → model
                  </li>
                  <li>
                    <span className="font-mono">Landed INR</span> → price
                  </li>
                  <li>
                    <span className="font-mono">Size US</span> → mapped to
                    UK/US/EU
                  </li>
                  <li>
                    <span className="font-mono">URL</span> → GOAT link
                  </li>
                  <li>
                    <span className="font-mono">image_urls</span> →
                    pipe-separated (optional)
                  </li>
                </ul>
              </div>
                ...
                <ul className="list-disc list-inside space-y-0.5 text-purple-600">
                  <li>
                    <span className="font-mono">
                      Sneaker / Brand / Silhouette / URL
                    </span>{" "}
                    → same
                  </li>
                  <li>
                    <span className="font-mono">
                      UK6 INR, UK6.5 INR … UK17 INR
                    </span>{" "}
                    → per-size price
                  </li>
                  ...
                  <li>
                    <span className="font-mono">image_urls</span> → shared
                    across all sizes
                  </li>
                </ul>
                <p className="text-purple-500 mt-1">
                  Sizes stored as <span className="font-mono">"uk 8.5"</span> —
                  brand-safe (no US/EU mapping needed).
                </p>
              </div>
            </div>
            <p className="text-xs text-gray-500 text-center">
              All imports set to <strong>active</strong>, condition{" "}
              <strong>new</strong>, category <strong>sneakers</strong>.
            </p>
          </CardContent>
        </Card>
              ...
              <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    className="rounded-xl border-0 glass-button text-gray-600 text-xs"
                    ...
                    <tr className="bg-white/30 text-gray-600 text-left">
                      <th className="px-3 py-2 font-semibold w-8">
                        <input
                          ...
                          checked={
                            selectedRows.length ===
                              rows.filter((r) => r.sizeValid && r.price > 0)
                            ...
                            {isMulti ? (
                              <span
                                ...
                                title={row.sizes
                                  .map(
                                    (s) =>
                                      `${s.ukLabel}: ₹${s.price.toLocaleString("en-IN")}`,
                                  )
                                  .join("\n")}
                              >
                                {row.raw_size_us}
                          ...
                          <td className="px-3 py-2">
                            {rowResult ? (
                              rowResult.status === "imported" ? (
                                ...
                                <Badge
                                  className="bg-yellow-100 text-yellow-700 border-0 rounded-lg text-xs"
                                  title={rowResult.reason}
                                >
                                ...
                                <Badge
                                  className="bg-red-100 text-red-700 border-0 rounded-lg text-xs"
                                  title={rowResult.reason}
                                >
                              ...
                              <Badge
                                className={`border-0 rounded-lg text-xs ${row.selected ? "bg-purple-100 text-purple-700" : "bg-gray-100 text-gray-500"}`}
                              >
