...
function buildOrderConfirmed(
  ...
  return `
    ...
    <!-- WhatsApp Community -->
    <div style="margin-top:28px;background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:2px solid #86efac;border-radius:16px;padding:24px;text-align:center;">
      <div style="font-size:22px;margin-bottom:8px;">💬</div>
      <p style="margin:0 0 6px;font-size:16px;font-weight:800;color:#15803d;">Join Our WhatsApp Community</p>
      <p style="margin:0 0 16px;color:#374151;font-size:14px;line-height:1.6;">
        Get <strong>exclusive drop alerts</strong>, early access deals, and connect with fellow sneaker heads. Be the first to know!
      </p>
      <a href="https://chat.whatsapp.com/DqDlG8APM0BFjKRmAI8Eb5"
         style="display:inline-flex;align-items:center;gap:10px;background:#25d366;color:#fff;font-weight:700;font-size:15px;text-decoration:none;padding:13px 28px;border-radius:50px;box-shadow:0 4px 12px rgba(37,211,102,0.35);">
      ...
      <p style="margin:12px 0 0;color:#6b7280;font-size:12px;">Free to join · Get notified before anyone else</p>
    </div>
  `;
}
...
function buildPreOrderLive(
  ...
  return `
    <h2 style="margin:0 0 4px;font-size:24px;font-weight:800;color:#111827;">🔥 Pre-Orders Are Live!</h2>
    <p style="margin:0 0 24px;color:#6b7280;font-size:15px;">Hi ${name}, great news! Pre-orders for <strong>${order.product_title}</strong> are now officially open!</p>

    <!-- Pre-Order info banner -->
    <div style="background:linear-gradient(135deg,#f3e8ff,#e9d5ff);border:1.5px solid #c084fc;border-radius:12px;padding:14px 18px;margin-bottom:24px;">
      <p style="margin:0;color:#6b21a8;font-size:14px;font-weight:800;">📦 Limited Pre-Order Window</p>
      <p style="margin:2px 0 0;color:#7e22ce;font-size:12px;">Estimated delivery: 28–35 days from order placement</p>
    </div>
    ...
    <!-- Product card -->
    <div style="background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:14px;padding:20px;margin-bottom:24px;">
      <p style="margin:0 0 4px;font-size:12px;font-weight:600;color:#7c3aed;text-transform:uppercase;letter-spacing:0.5px;">Pre-Order Item</p>
      <p style="margin:0;font-size:18px;font-weight:800;color:#111827;line-height:1.3;">${order.product_title}</p>
      ${variantHtml}
      ${sizeHtml}
      <p style="margin:12px 0 0;font-size:18px;font-weight:900;color:#7c3aed;">${formatAmount(order.amount, order.currency)}</p>
    </div>

    ${ctaButton("Pre-Order Now", actionUrl)}

    <p style="margin:16px 0 0;color:#9ca3af;font-size:13px;text-align:center;">
      Pre-order window is open for a limited time. Secure your pair now!
    </p>
...
function buildEmailContent(req: EmailRequest): { subject: string; html: string } {
  ...
  switch (type) {
    ...
    case "payment_reminder":
      ...
      break;
    case "preorder_live":
      body = buildPreOrderLive(recipient_name, order_data, actionUrl);
      break;
...
function getDefaultSubject(type: string, productTitle: string): string {
  switch (type) {
    ...
    case "preorder_live":     return `🔥 Pre-Order Open: ${productTitle} — Reserve Yours Now!`;
