-- Add is_instant_ship column to both size tables so individual sizes can be
-- marked as in-hand (instant ship) while others are imported (3-4 weeks).

ALTER TABLE product_listing_sizes
  ADD COLUMN IF NOT EXISTS is_instant_ship BOOLEAN NOT NULL DEFAULT FALSE;

ALTER TABLE product_variant_sizes
  ADD COLUMN IF NOT EXISTS is_instant_ship BOOLEAN NOT NULL DEFAULT FALSE;

-- ── Rebuild views to expose has_instant_ship ───────────────────────────────
-- Drop dependents first (most-derived first)
...
-- hot_deals_with_images: curated by admin (is_hot_deal flag + 30%+ discount)
...
-- instant_shipping_with_images: any size marked instant ship OR legacy delivery_days < 10
...
-- ── Rebuild browse_all_listings to use has_instant_ship ───────────────────
