-- Fix browse_all_listings to use multi-word token search instead of literal
-- phrase match. "SB Dunk" is now split into ["SB", "Dunk"] and every token
-- must appear somewhere in title, brand, or description (AND logic, any order).
-- This matches the old buildMultiWordSearchFilter behaviour in Browse.tsx.
