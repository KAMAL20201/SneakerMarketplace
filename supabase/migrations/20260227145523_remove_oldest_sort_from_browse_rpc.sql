-- Remove 'oldest' sort option from browse_category_listings RPC.
-- The "Oldest First" sort is no longer exposed in the UI so the corresponding
-- ORDER BY clause is removed. Any call with p_sort = 'oldest' will now fall
-- through to the default (newest-first, created_at DESC) gracefully.
...
BEGIN
  ...
  ORDER BY
    ...
    END DESC NULLS LAST,
    -- Default (newest) and any unrecognised p_sort value → created_at DESC.
