-- Fix browse_all_listings search to use prefix-token matching instead of
-- whole-word boundary matching.  With the previous `\m...\M` pattern a
-- partial query like "jord" would NOT match "Jordan" because `\M` requires
-- the pattern to end at a word boundary.  Switching to `\m` (start-of-word)
-- without `\M` makes every search token a prefix match, so "jord" now
-- matches "Jordan", "Jordans", etc.
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE l.status = 'active'
      ...
      -- Prefix-token search: every token must appear as a word-prefix
      -- (using \m start-of-word anchor WITHOUT \M end-of-word anchor)
      AND (
