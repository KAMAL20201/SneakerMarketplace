-- ROLLBACK SCRIPT — only run this if 20260411000001 caused problems.
--
-- What it does:
--   1. Recreates browse_category_listings WITHOUT slug (pre-slug version,
--      with discount-high sort intact).
--   2. Recreates browse_all_listings WITHOUT slug (pre-slug version).
--   3. Recreates listings_with_images WITHOUT slug.
--   4. Recreates hot_deals_with_images WITHOUT slug.
--   5. Drops the slug column from product_listings (and its index/trigger).
--
-- What it does NOT touch:
--   - Any other tables, columns, or data.
--   - The slugify() helper function (harmless to leave).
--
-- After running this, the app reverts to the pre-slug state.
-- CategoryBrowse and Browse will fall back to listing.id for navigation
-- (the `listing.slug ?? listing.id` guard in the frontend handles this).

-- ── 1. browse_category_listings — restore pre-slug version ───────────────────
...
-- ── 2. browse_all_listings — restore pre-slug version ────────────────────────
...
-- ── 3. listings_with_images — restore pre-slug version ───────────────────────
...
-- ── 4. hot_deals_with_images — restore pre-slug version ──────────────────────
...
-- ── 5. Remove slug column from product_listings ───────────────────────────────
DROP TRIGGER IF EXISTS trg_listing_slug ON product_listings;
DROP INDEX  IF EXISTS idx_product_listings_slug;
ALTER TABLE product_listings DROP COLUMN IF EXISTS slug;
